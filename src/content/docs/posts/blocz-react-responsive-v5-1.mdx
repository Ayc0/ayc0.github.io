...
Before the 5.1.0, the word `breakpoint` was used for describe the defined _sizes_ available to all our utilities functions. But this name never sat right with me. Why? Because a breakpoint really is just a point, but when you looked at our documentations, we were defining those as ranges:
...
Now, if your app was defined breakpoint providers, it could behave differently. For instance:
...
It’s a bit weird to introduced a deprecated API. But this is done so that every renaming could be done in 1 go, and then the migration away from the context could be done too.
